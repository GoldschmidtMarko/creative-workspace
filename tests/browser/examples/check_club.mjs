...
// Teams is the default tab (the primary journey — clicking into a team) —
// scoped to #teams-body since a bare ".pl-empty" also matches the (hidden)
// #search-status element elsewhere on the page and never resolves.
await page.waitForSelector("#teams-body .team-grid, #teams-body .pl-empty", { timeout: 30000 });
console.log("teams tab loaded OK (default)");

await page.click('.subnav__tab[data-tab="roster"]');
await page.waitForSelector(".club-roster tbody tr", { timeout: 20000 });
console.log("roster tab loaded OK");
